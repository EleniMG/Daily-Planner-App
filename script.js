...
saveButton.on("click", function updateSchedule(event){
    ...
    inputValue = $('input[name="task-entry"]').val().trim();
    schedule[Number(event.target.id)].task = inputValue;
    localStorage.setItem("schedule", JSON.stringify(schedule))

    var updatedSchedule = JSON.parse(window.localStorage.getItem("schedule"));
